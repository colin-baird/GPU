...
void DivideUnit::accept(const DispatchInput& input, uint64_t cycle) {
    ...
    busy_.set_next(true);
    cycles_remaining_.set_next(DIVIDE_LATENCY);
    ...
    pr.source_unit = ExecUnit::DIVIDE;
    pr.pc = input.pc;
...
void DivideUnit::commit() {
    ...
    // holds — every Reg holds, so the next tick's seed_next()+evaluate()
    // re-derive the identical countdown step and (if the countdown reached 0)
    // the identical result. The countdown is decremented at most once per
    // non-stalled cycle.
