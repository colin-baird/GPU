...
TEST_CASE("Cache: load miss then hit", "[cache]") {
    ...
    bool accepted = f.cache.process_load(256, 0, 5, results, 1, wb);
    REQUIRE(accepted);
    ...
    WritebackEntry wb2;
    wb2.valid = false;
    bool accepted2 = f.cache.process_load(256, 0, 6, results, 2, wb2);
    REQUIRE(accepted2);
...
TEST_CASE("Cache: store hit writes to write buffer", "[cache]") {
    ...
    f.cache.process_load(0, 0, 1, results, 1, wb);
    f.tick_mem(10);
    WritebackEntry fill_wb;
    bool fill_valid;
    f.cache.handle_responses(fill_wb, fill_valid);
    ...
    bool accepted = f.cache.process_store(line_addr);
    REQUIRE(accepted);
...
TEST_CASE("Cache: MSHR stall when all entries used", "[cache]") {
    ...
    bool accepted = f.cache.process_load(addr, 0, 10, results, 1, wb);
...
TEST_CASE("Cache: direct-mapped conflict eviction", "[cache]") {
    ...
    uint32_t addr_a = 0;
    f.cache.process_load(addr_a, 0, 1, results, 1, wb);
    f.tick_mem(10);
    WritebackEntry fill;
    bool fill_valid;
    ...
    // Verify addr_a hits
    wb.valid = false;
    f.cache.process_load(addr_a, 0, 1, results, 2, wb);
    ...
    wb.valid = false;
    f.cache.process_load(addr_b, 0, 2, results, 3, wb);
    ...
    wb.valid = false;
    f.cache.process_load(addr_a, 0, 3, results, 4, wb);
...
TEST_CASE("Cache: write buffer full stalls store", "[cache]") {
    ...
    for (uint32_t i = 0; i < WB_DEPTH; ++i) {
        bool accepted = f.cache.process_store(0); // line_addr 0
        REQUIRE(accepted);
    }

    // Write buffer full: next store should stall
    bool accepted = f.cache.process_store(0);
    REQUIRE_FALSE(accepted);
...
TEST_CASE("Cache: reset clears all state", "[cache]") {
    ...
    // Verify it hits
    wb.valid = false;
    f.cache.process_load(0, 0, 1, results, 2, wb);
    REQUIRE(wb.valid);
    ...
    // Now it should miss
    wb.valid = false;
    f.cache.process_load(0, 0, 1, results, 3, wb);
