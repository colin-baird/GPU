...
(generated by `cmake -B build`) and the libclang Python bindings. If
either is unavailable the layer is skipped with a clear diagnostic
rather than failing — the header-naming layers still run.
...
def lint_cross_module_reads(
    ...
    """Cross-module-read layer. Returns (findings, skip_notes).

    A non-empty `skip_notes` with empty `findings` means the layer could
    not run (missing libclang bindings or compile DB) — the caller
    surfaces the note but does not fail the build on it.
    """
    skip_notes: list[str] = []
    try:
