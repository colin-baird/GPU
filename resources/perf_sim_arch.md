...
- **Struct `BufferEntry`**: `decoded` (DecodedInstruction), `warp_id`, `pc`, `prediction`.
- **Class `InstructionBuffer`**: `RegFifo<BufferEntry>`-backed FIFO with fixed `max_depth`. Phase 6 (sparkling-dazzling-starfish.md) brought the buffer onto full RegFifo discipline: production callers `stage_push` (from `DecodeStage::evaluate`) and `stage_pop` (from `WarpScheduler::evaluate`) intents; the per-warp `commit(bool flush_request)` driven from `TimingModel::tick()`'s commit phase applies pop-then-push atomically, or clears the buffer when `flush_request` is true. The flush request is a per-warp `Wire<std::array<bool, MAX_WARPS>>` owned by `TimingModel` and driven by `FetchStage::apply_redirect()` — the literal hardware-faithful encoding of a mispredict-resolve flush (staged at evaluate, applied at the cycle boundary). Committed-state reads: `is_full()`, `is_empty()`, `size()`, `capacity()`, `front()`. Test-only convenience immediate-apply helpers `push()` / `pop()` / `flush()` are preserved for bypass-tick() fixture setup; production no longer uses them. `InstructionBuffer` intentionally does not derive `RegisteredStage` (it is a member of `WarpState`, which is held by value in `std::vector<WarpState>` and so must remain movable); the parameterized `commit(bool)` is its lifecycle entry point.
...
- **`set_instr_buffer_flush_request(req_ptr)`**: Phase 6 (sparkling-dazzling-starfish.md) wiring. Called once during `TimingModel` construction with a pointer to `TimingModel::instr_buffer_flush_request_` so `apply_redirect()` drives the per-warp bit for the warp it is flushing; the per-warp `instr_buffer.commit()` at the end of this tick honors the wire. Null-tolerant: tests that exercise `FetchStage` in isolation see the wire unwired and `apply_redirect()` falls back to the buffer's immediate `flush()` test helper.
...
- **`tick()`** -> `bool` (continue?): One cycle of simulation. See the "Tick discipline" section of `resources/timing_discipline.md` for the full contract. Steps:
  ...
  4. **Commit phase**: every stage flips `next_* -> current_*`; the five units, `opcoll_`, and `scheduler_` self-gate on `wb_arbiter_->next_writeback_stall()` and hold on a stalled cycle. `scoreboard_.commit()` / `branch_tracker_.commit()` run unconditionally. Per-warp `instr_buffer.commit(flush_bit)` runs over every warp, reading the matching bit of `instr_buffer_flush_request_` (Phase 6 of sparkling-dazzling-starfish.md — drives a same-tick clear when fetch.apply_redirect asserted it for that warp; otherwise applies the staged pop-then-push pair from scheduler / decode). Per-warp `WarpState::commit()` flips `pc_` / `active_`. `commit_cross_stage_fifos()` runs at the end of this phase to apply the addr-gen FIFO's pop-then-push atomically (ungated by construction).
