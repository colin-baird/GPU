...
- **Write-ack line pinning**: a queued write-through pins its cache set until the external write ack returns. `is_pinned(set)` = chain pin (`current_tags_[set].pinned`) OR write-ack pin (`current_outstanding_writes_[set] > 0`); both are REGISTERED `current_`/`next_` pairs. `queue_write_through(line_addr)` is the single, **fallible** write-buffer enqueue wrapper — the write buffer has one enqueue port per cycle. The first caller claims the port (`next_write_buffer_port_claimed_`), stages the push (`next_write_buffer_push_`, applied at commit), bumps the per-set `next_outstanding_writes_` and global `next_outstanding_writes_total_`, and returns true; a later caller this cycle finds the port claimed and returns false without staging anything. The three enqueue sites (store-miss fill, store-secondary drain, store hit — FILL > secondary > HIT by tick order) call it after confirming admission (write-buffer depth AND `outstanding_writes_at_cap()`), and perform any tag install / MSHR free / pin clear only on a true return; a port-busy false routes into the existing backpressure path (`complete_fill` defers, `drain_secondary_chain_head` leaves the secondary, `process_store` retries — no `stalled_`/`stall_reason_`, quantified by `write_buffer_port_conflict_cycles`). A cap refusal routes into the same write-buffer-full backpressure path (stall reason `WRITE_BUFFER_FULL`, attributed to `write_throttle_stall_cycles`). The three pin-enforcement sites (`process_load`/`process_store` miss pin-check, `complete_fill` deferred-fill guard) use `is_pinned`; a stall is attributed to `line_pin_stall_cycles` when the chain pin is the cause, else `write_ack_pin_stall_cycles` (precedence chain > write-ack).
- **`handle_responses()`**: First, unconditionally consumes at most one **write ack** from `mem_if_`'s separate write-ack channel — decrementing the per-set and global outstanding-write counters — *before* the deferred-fill early return, so a fill deferred on a write-ack pin always makes progress (the deadlock fix). Then processes at most one read fill per cycle: for load fills, deposits lane values into the owning warp's gather buffer via `gather_file_.try_write(... FILL)`; for store fills, queues a write-through. Read fills are buffered internally (`pending_fill_`) if a store fill is blocked by the write buffer or a conflicting fill is deferred by a pin.
- **Registered write-buffer FIFO**: `write_buffer_` is a registered, single-enqueue-port FIFO mutated **only** by `commit()`. Evaluate-phase reads (`write_buffer_size()`, `.empty()`, `.front()`, depth checks) read the committed deque. `commit()` applies the staged ops — pop (`next_write_buffer_pop_`) then push (`next_write_buffer_push_`) — and clears the staging slots and the port-claim flag. It is therefore not fall-through: a line enqueued at cycle T is submittable no earlier than T+1.
- **`drain_write_buffer()`**: Reads the committed front of the write buffer, stages it to memory via `mem_if_.set_next_write_request(...)` (gated on `!mem_if_.next_request_stall()`), and **stages** the pop (`next_write_buffer_pop_ = true`) instead of popping directly — the entry leaves `write_buffer_` at `commit()`. The COMBINATIONAL backward stall is the architectural backpressure path: `DRAMSim3Memory` raises `next_request_stall()` when its write-region FIFO is at capacity, and the entry stays at the buffer's head until the stall clears (`FixedLatencyMemory::next_request_stall()` is hardwired false — unbounded queue). Silently popping while stalled would lose the write (the timing-only model has no recovery).
- **`evaluate()`**: Resets the COMBINATIONAL stall scratch (`stalled_` / `stall_reason_`) to `NONE` and the fill-conflict scratch `fill_installed_set_` to `-1`, seeds the REGISTERED scratch (clears `next_last_*_event_` slots; carries `current_pending_fill_` forward into `next_pending_fill_`; seeds `next_tags_ = current_tags_`, the outstanding-write counters `next_outstanding_writes_` / `next_outstanding_writes_total_`, and the registered MSHR file via `mshrs_.seed_next()`), then runs `handle_responses()` (FILL) followed by `drain_secondary_chain_head()` (secondary). Phase 7: the prior `gather_extract_port_used_` cache-side scratch flag is removed; FILL > secondary > HIT priority is encoded by tick ordering — `cache_->evaluate()` runs at the top of the non-panic tick (FILL then secondary), and `coalescing_->evaluate()` runs later in the tick (HIT via `process_load`). The shared port itself is arbitrated by `LoadGatherBufferFile::try_write` reading its REGISTERED `next_port_claimed_` flag.
- **`commit()`**: Phase 9 boundary discipline. Flips the REGISTERED next/current pairs (the tag array `tags_`, `pending_fill_`, the outstanding-write counters `outstanding_writes_` / `outstanding_writes_total_`, the four `last_*_event_` slots), flips the registered MSHR file via `mshrs_.commit()`, and applies the registered write-buffer FIFO's staged ops (pop then push, then clears the staging slots and `next_write_buffer_port_claimed_`) — so post-commit observers read consistent committed values via the `current_*` accessors. The COMBINATIONAL same-tick scratch (`stalled_`, `stall_reason_`, `fill_installed_set_`) is not pipeline state; `fill_installed_set_` is additionally cleared here to bound its lifetime to one tick. See inventory rows 9, 10 in `resources/timing_discipline.md`.
...
- **`is_idle()`**: True only when there are no live MSHRs, no queued write-through entries, no pending fill, no in-flight command, and `current_outstanding_writes_total_ == 0` (a write that left the write buffer but is not yet acked keeps the cache non-idle).
...
- **Class `MSHRFile`**: REGISTERED (double-buffered) MSHR file — `current_entries_` / `next_entries_`. `allocate(entry)` scans `current_entries_` for a free slot and writes the entry into `next_entries_`, returning the index (or -1). `free(index)` clears `next_entries_[index].valid`. Readers `has_free()`, `has_active()`, `find_chain_tail(line_addr)` scan committed `current_entries_`. `at()` is split into `current_at(index)` (committed read) and `next_at(index)` (mutable next-state write handle, e.g. a chain-link write). `seed_next()` copies current→next; `commit()` flips next→current — both driven by `L1Cache` (`seed_next()` from `evaluate()`'s seeding block, `commit()` from `L1Cache::commit()`). Consequence: a slot freed this cycle is not reusable until next cycle, and a chain tail freed this cycle stays visible to a same-cycle `find_chain_tail`. The lint treats `MSHRFile` as an internal helper buffer (out of scope for the cross-stage accessor convention; its `next_at` is an intra-module registered-pair write handle).
...
- **Two bounded completion queues**: read fills land on `responses_` (capacity `read_response_queue_capacity_ = num_mshrs`); write acks land on the separate `write_acks_` deque (capacity `write_ack_queue_capacity_ = max_outstanding_writes + chunks_per_line`). `on_read_complete` / `on_write_complete` assert against their respective bounds. The `worst-case cache traffic` Catch2 case in `test_dramsim3_memory.cpp` drives peak production and verifies both bounds hold (snapshots via `max_observed_response_queue()` / `max_observed_write_ack_queue()`).
