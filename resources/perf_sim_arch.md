...
- **Issue-scoreboard constants**: `kUnitIterationLatency[]` (per-unit structural-hazard latency: `DIVIDE`=32, `TLOOKUP`=17, fully-pipelined `ALU`/`MULTIPLY`/`LDST`=0), `kIssueToWritebackOffset[]` (end-of-Phase-10B issue→arbiter-consume offsets: ALU=3, MULTIPLY=`kMulPipelineStages`+2, DIVIDE=`kDivideLatency`+2, TLOOKUP=`kTlookupLatency`+2), `kWritebackBitmapLen`, and `compute_issue_to_writeback_offset()` (adds 1 for VDOT8). Used by `WarpScheduler` (10B.0).
...
- **`WarpScheduler(num_warps, warps_ptr, func_model_ref, stats_ref)`**
- **Issue scoreboard (Phase 10B.0)**: the scheduler predicts unit and writeback-port availability from its own *issue history* rather than polling downstream `current_busy()` signals. Fields:
  ...
  - **`writeback_bitmap_` + `bitmap_head_`** — the binding fixed-latency writeback schedule. A circular array; a fixed-latency op that writes back reserves the slot at `bitmap_head_ + compute_issue_to_writeback_offset(...)`; the issue gate refuses a colliding reservation. `bitmap_head_` advances one slot per non-frozen cycle. Fixed-latency reservations only; the arbiter does not consult it.
