...
- **Class `LoadGatherBufferFile : public ExecutionUnit, public RegisteredStage`**: Allocates `num_warps` buffers indexed by `warp_id` and registers with the writeback arbiter. Phase 10D: the fill presentation is genuinely **double-buffered** — the committed buffers are the state read by `consume_result()` / `buffer()` / `current_busy()`; the staged buffers accumulate this cycle's `try_write()` fills and the staged claim/release. This makes the cache → gather → writeback-arbiter forward edge a REGISTERED edge whose result is independent of the arbiter's sweep position. The gather buffer is NOT frozen by the writeback stall — it `seed_next()`s and `commit()`s every cycle. Phase 5b (reg.h migration): the file derives `RegisteredStage` and holds every registered field as a `Reg<T>` — `buffers_` is `Reg<std::vector<LoadGatherBuffer>>`, `claim_request_` is `Reg<GatherClaimRequest>`, `has_result_` is `Reg<bool>`, `rr_pointer_` is `Reg<uint32_t>`. The two non-register slots are `next_port_claimed_` (Phase 7: `Wire<bool>` for the combinational, intra-tick port arbitration — intra-class signal, no public accessor; `drive()` from `try_write()`, `reset()` at the OWNER'S `commit()` so the claim persists across the whole tick and only clears at the cycle boundary) and `next_release_` (an evaluate-staging slot that carries a payload, not a level signal — stays plain because consume_result writes it and the same tick's commit consumes-and-clears it).
  ...
  - **`commit()`** (Phase 5b): resets the `Wire<bool> next_port_claimed_` (Phase 7: same intra-tick lifetime as pre-Phase-7 — the wire is reset here, not at evaluate top); applies the staged release into `buffers_.next_mut()` and `rr_pointer_.next_mut()`; recomputes the staged `has_result_` from `buffers_.next()`; calls `commit_all()` to flip every enrolled Reg (`buffers_`, `rr_pointer_`, `claim_request_`, `has_result_`) in one sweep; then re-clears `claim_request_` via `set_next(GatherClaimRequest{})` so a subsequent `commit()` with no intervening `claim()` does not re-latch a stale value (matches the pre-Phase-5b `next_claim_request_ = GatherClaimRequest{}` tail-of-commit clear; same memoryless-consumer pattern as L1Cache's `load_cmd_` / `store_cmd_`).
  - **`reset()`** (Phase 5b): `reset_all()` value-initializes every Reg; the `buffers_` inner vector is then re-sized back to `num_warps_` via the `set_next` + `commit` + `set_next` pattern (same Phase 5a precedent used for L1Cache's `tags_` and `outstanding_writes_`); the `Wire<bool> next_port_claimed_` is reset via `wire_.reset()` (Phase 7), and the plain `next_release_` staging slot is cleared by hand.
