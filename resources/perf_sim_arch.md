...
Stages that expose a READY/STALL signal add a `const` accessor (e.g. `ready_out()` or `ready_to_consume_fetch()`) that reads only their own committed (`current_*`) state. The accessor is queried by upstream producers from inside their own `evaluate()`; because the value is committed-state-derived, it is stable across the entire evaluate phase regardless of where in the sweep the producer sits. See `resources/timing_discipline.md`.
...
- **Class `ExecutionUnit`**: A separate hierarchy from `PipelineStage` (units have a different lifecycle — they produce results consumed by `WritebackArbiter` rather than participating in the unified evaluate/commit fan-in), but it shares the same convention: pure-virtual `ready_out()` (a `const` accessor reading the unit's own committed `current_*` state), `evaluate()`, `commit()`, `reset()`, plus the unit-specific `has_result()`, `consume_result()`, and `get_type()`. READY/STALL discipline: `WarpScheduler::evaluate()` reads each unit's `ready_out()` directly during its own evaluate; the same accessor is reused by post-commit drain checks (`pipeline_drained` / `execution_units_drained` / `trace_cycle`) since both contexts read committed state.
...
- **Event-shaped writers** (all write to `next_`):
  ...
  `OperandCollector::resolve_branch()` when the branch resolves with
  prediction == actual; clears immediately because no flush is needed.
  `note_redirect_applied(w)` — called by `FetchStage::apply_redirect()`
  when a mispredict-redirect actually lands at fetch's commit phase;
  the deferred clear means the scheduler keeps observing
...
- **`ready_to_consume_fetch()`**: `const` accessor reading `!pending_.valid`. Consumed by `FetchStage::evaluate()` as the READY/STALL backpressure gate; tick order ensures fetch's evaluate runs before decode's evaluate, so the value fetch reads reflects committed state.
...
- **`commit()`**: First latches the Phase 6 REGISTERED ebreak slot (`current_ebreak_request_ = next_ebreak_request_`). Then applies any Phase 5 REGISTERED redirect via the `read_redirect_request(redirect_override_, opcoll_)` helper: if valid and the pending entry's warp matches, drops the pending entry. Then pushes the (possibly cleared) staged instruction to the target warp's instruction buffer once space is available; otherwise the staged decode remains pending (and `ready_to_consume_fetch()` will report `false` next cycle). The redirect-apply is sequenced before the push so a shadow instruction whose pending entry would otherwise have just landed in the warp buffer is dropped instead.
...
1-cycle ALU execution unit. Phase 1 cross-stage signaling discipline: `result_buffer_`, `has_pending_`, `pending_input_`, and `pending_cycle_` are next/current double-buffered. `accept()`, `evaluate()`, and `consume_result()` write only `next_*`; `commit()` flips `next_* -> current_*` at the cycle boundary. `ready_out()` reads committed (`current_*`) state for the scheduler; `has_result()` reads the live (`next_*`) result buffer for the COMBINATIONAL same-tick edge with the writeback arbiter.

- **`ALUUnit(stats_ref)`**
- **`ready_out()`**: `const` accessor returning `!current_result_buffer_.valid && !current_has_pending_`. Queried by the scheduler this cycle and reused by post-commit drain checks.
...
- **`ready_out()`**: `const` accessor reading committed state. Pipeline is "stalled" iff `current_result_buffer_.valid && !current_pipeline_.empty() && current_pipeline_.front().cycles_remaining == 0`; otherwise ready.
...
- **`ready_out()`**: `const` accessor returning `!current_busy_ && !current_result_buffer_.valid`.
...
- **`process_load(addr, warp_id, lane_mask, results, issue_cycle, pc, raw_instruction)`**: Direct synchronous API for tests. Hit -> calls `gather_file_.try_write()` for the lanes selected by `lane_mask`. Phase 7: arbitration is owned by `LoadGatherBufferFile` (single REGISTERED port-claim flag); `try_write()` returns false when the port was already claimed earlier in the same tick by an in-cycle FILL or secondary drain — caller must retry next cycle. Miss -> allocates an MSHR recording `lane_mask`, submits the read, and records a miss-allocation trace event. Returns false if MSHR full (stall). Does not produce a writeback directly. Phase M3: production coalescing path no longer calls this directly — see `set_next_load_cmd` and the cmd-stall machinery below.
- **`process_store(line_addr, warp_id, issue_cycle, pc, raw_instruction)`**: Direct synchronous API for tests. Hit -> updates cache, pushes to write buffer. Miss -> allocates MSHR (write-allocate) with trace metadata. Returns false if MSHR or write buffer full. Phase M3: production coalescing uses `set_next_store_cmd` instead.
- **Phase M3 — REGISTERED forward command path**: `set_next_load_cmd(addr, warp_id, lane_mask, results, issue_cycle, pc, raw_instruction)` and `set_next_store_cmd(line_addr, warp_id, issue_cycle, pc, raw_instruction)` stage a cmd in `next_load_cmd_` / `next_store_cmd_`. `commit()` flips next → current; `evaluate()` consumes current after handle_responses (FILL) and drain_secondary_chain_head (secondary). On processing failure (port lost, MSHR full, pin/wb conflict), the cmd stays valid and retries next cycle. `next_cmd_stall()` is the COMBINATIONAL backward stall: returns true while a cmd is in flight, when MSHRs are exhausted, or when the write buffer is full. Coalescing reads `next_cmd_stall()` same-cycle (cache.evaluate runs first) and bails before staging if true. `next_cmd_stall_reason()` returns the structural reason (MSHR_FULL / WRITE_BUFFER_FULL / LINE_PINNED) for trace classification surfacing WAIT_L1_MSHR / WAIT_L1_WRITE_BUFFER from the LDST FIFO head when coalescing is preempted.
...
- **`evaluate()`**: Resets the COMBINATIONAL stall scratch (`stalled_` / `stall_reason_`) to `NONE`, seeds the REGISTERED scratch (clears `next_last_*_event_` slots; carries `current_pending_fill_` forward into `next_pending_fill_`), then runs `handle_responses()` (FILL) followed by `drain_secondary_chain_head()` (secondary). Phase 7: the prior `gather_extract_port_used_` cache-side scratch flag is removed; FILL > secondary > HIT priority is encoded by tick ordering — `cache_->evaluate()` runs at the top of the non-panic tick (FILL then secondary), and `coalescing_->evaluate()` runs later in the tick (HIT via `process_load`). The shared port itself is arbitrated by `LoadGatherBufferFile::try_write` reading its REGISTERED `next_port_claimed_` flag.
- **`commit()`**: Phase 9 boundary discipline. Flips the REGISTERED next/current pairs (`pending_fill_`, the four `last_*_event_` slots) so post-commit observers — `TimingModel::record_cycle_trace`, tests asserting cycle-end state — read consistent committed values via the `current_*` accessors. The COMBINATIONAL stall slots (`stalled_`, `stall_reason_`) and internal hardware state (`tags_`, `mshrs_`, `write_buffer_`) are not flipped here; they're either same-cycle scratch or direct-mutated synchronously. See inventory rows 9, 10 in `resources/timing_discipline.md`.
...
- **Class `LoadGatherBufferFile : public ExecutionUnit`**: Allocates `num_warps` buffers indexed by `warp_id` and registers with the writeback arbiter. Owns the gather-extract port arbitration via a single shared `next_port_claimed_` flag — first writer in a tick wins; `commit()` clears it at end-of-cycle. Models the spec §5.3 "one line-to-gather-buffer extraction per cycle" port shared by FILL, secondary drain, and HIT.
  - **`current_busy(warp_id)`**: Query used by the coalescing unit to gate claiming a load.
  - **`claim(warp_id, dest_reg, pc, issue_cycle, raw_instruction)`**: Phase M2: REGISTERED. Stages a `GatherClaimRequest` in `next_claim_request_`; the buffer mutation is deferred. `commit()` flips next_ → current_; `evaluate()` (called at top of each tick, before `cache.evaluate`) consumes `current_claim_request_` and applies metadata + `buf.busy=true`. Same-cycle FILL/secondary writes via `try_write` therefore observe the freshly-applied claim. The single-slot design is sufficient because production protocol guarantees at most one claim per cycle (coalescing processes one FIFO entry at a time and only loads claim).
  - **`evaluate()`**: Phase M2: applies any pending claim from `current_claim_request_` if valid, then clears the valid bit. Runs at the top of every tick (before `cache.evaluate`).
  - **`try_write(warp_id, lane_mask, values, source)`** where `source` is `HIT` or `FILL`: First check is the live `next_port_claimed_` flag; if already set this tick, returns false and (for `HIT` source) increments `stats_.gather_buffer_port_conflict_cycles`. Otherwise writes the selected lanes into the buffer, increments `filled_count`, and sets `next_port_claimed_ = true`. Within a single tick the cache calls `try_write` in tick-order — FILL (handle_responses) -> secondary (drain_secondary_chain_head) -> HIT (process_load via coalescing) — so the first writer wins and encodes the FILL > secondary > HIT priority. `commit()` clears the flag at end-of-cycle.
...
- **`tick()`** -> `bool` (continue?): One cycle of simulation. Forward-order evaluation:
  ...
  3. `cache_.evaluate()`
  4. **Forward sweep (evaluate)**: `fetch_` -> `decode_` (writes `next_ebreak_request_` if it sees an EBREAK; commit at step 6 latches it for next tick's observation) -> `scheduler_` -> `opcoll_` -> dispatch -> branch resolution (Phase 5: `opcoll_->resolve_branch(warp_id, mispredicted, target_pc)` writes the REGISTERED redirect slot and clears `branch_tracker_` on correct prediction; on mispredict, the tracker clear is deferred to `FetchStage::commit()`) -> execute (all units) -> `coalescing_` -> `mem_if_` -> MSHR fill -> write buffer drain -> `wb_arbiter_`. READY/STALL signals are queried inline as `const` accessors on the consumer's committed state: `fetch_.evaluate()` reads `decode_.ready_to_consume_fetch()` for backpressure; `scheduler_.evaluate()` reads `opcoll_->ready_out()` and each unit's `ready_out()` directly. Each accessor reads only the consumer's `current_*`, so the value is stable through the entire evaluate phase regardless of where in the sweep it is queried. Phase-5: the prior mid-tick `fetch_->redirect_warp(...)` and `decode_->invalidate_warp(...)` calls are gone — `FetchStage::commit()` and `DecodeStage::commit()` read `opcoll_->current_redirect_request()` and apply the flush from there (one-cycle delay vs. pre-Phase-5). Phase-6: the prior mid-tick `decode_->ebreak_detected()` early-return arm is gone; the trigger now happens at step 2 of the *next* tick and the flush cascade happens at the commit-phase boundary (step 6).
  5. All `.commit()` (including `branch_tracker_.commit()` after `scoreboard_.commit()`).
  6. **Phase 6 panic-flush cascade**: if `pending_panic_flush_` is armed, call `scheduler_->flush()`, `opcoll_->flush()`, `gather_file_->flush()`, `wb_arbiter_->flush()`, and `branch_tracker_.reset()`. Each `flush()` delegates to the corresponding `reset()`; the call site moves from mid-evaluate to commit-phase to fix the discipline violation.
  7. Termination check: `all_warps_done() && pipeline_drained()`. When panic is active (separate top-of-tick branch), the controller queries the wired drained_query_ callable inside its own `evaluate()` and the early-return `is_done()` arm exits the simulation.
