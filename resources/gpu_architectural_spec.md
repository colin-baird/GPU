...
**Simulator memory backends.** The performance simulator selects between two backends behind the same `ExternalMemoryInterface` (`set_next_read_request`, `set_next_write_request`, `next_request_stall`, `evaluate`, `commit`, `is_idle`, `current_has_response`, `get_response`, plus the test-direct `submit_read`/`submit_write` for backend-isolation tests) via `SimConfig::memory_backend`. The cache stages requests via the REGISTERED `set_next_*_request` setters, gated on the COMBINATIONAL backward `next_request_stall()` (DRAMSim3 raises it when its write-region FIFO is at capacity; FixedLatencyMemory hardwires it false). `commit()` flips next_*_request_ → current_*_request_; the next `evaluate()` admits to `in_flight_`.
