...
- **Registered tag array** (landed): the L1 tag array `tags_` is promoted
  ...
  reads `current_tags_` like every other tag reader and pin-clears
  `next_tags_` (the write side). A secondary of the line whose primary
  fills this cycle cannot drain in the fill cycle regardless: the
  registered MSHR file keeps the primary valid in `current_entries_` until
  commit, so the head-detection scan gates the fill-cycle drain — the
  `current_tags_` read is therefore observationally exact and avoids a
  same-cycle read of `complete_fill`'s registered `next_tags_` write. This
  retires the implicit write-first tag-array assumption and
