...
1. **evaluate phase (forward sweep, dataflow order).** Each stage reads
   ...
   of upstream for COMBINATIONAL edges, or a downstream consumer's
   const `ready_out()` accessor for READY/STALL edges) and writes its
...
part of the discipline contract. READY/STALL signals are exposed as
...
The consumer exposes a `ready_out()` (or analogous) `const` accessor
that reads only its own `current_*`. The producer calls
`consumer.ready_out()` during its own `evaluate()` and uses the result
to decide whether to advance or hold. Modeling backpressure this way
matches a real ready/valid handshake: the stall signal flows backward
in the same cycle, but the underlying state from which it was computed
is committed, so the value is stable across the entire evaluate phase
regardless of where the producer is positioned in the sweep.

```
cycle N    : producer.evaluate() -> reads consumer.ready_out()
                                    (which reads consumer.current_*)
```

Used for: decode → fetch backpressure (`DecodeStage::ready_to_consume_fetch()`),
opcoll → scheduler backpressure (`OperandCollector::ready_out()`), unit →
scheduler backpressure (each `ExecutionUnit::ready_out()`),
gather-buffer write-port reservation.
...
|---|---|---|---|---|---|---|---|
| 1 | `DecodeStage::ready_to_consume_fetch()` (`const` accessor reading `pending_.valid`) | `FetchStage::evaluate` | `bool` decode-ready signal | **READY/STALL** | tick order: `fetch_.evaluate` -> `decode_.evaluate` (so the value fetch reads is committed-state stable) | compliant | 3 |
...
| 3 | `OperandCollector::ready_out()` (`const` accessor reading `current_busy_`) | `WarpScheduler::evaluate` (reads `opcoll_->ready_out()` directly via wired pointer) | `bool` opcoll-free flag | **READY/STALL** | the accessor reads only `current_busy_`, so its value is stable across the entire evaluate phase regardless of where the scheduler queries it; `set_opcoll_free` setter deleted | compliant | 4 |
| 4 | Each `ExecutionUnit::ready_out()` (`const` accessor on the base; each concrete unit reads its own `current_*`) | `WarpScheduler::evaluate` (reads `unit->ready_out()` via typed pointers wired by `set_consumers`) | per-unit ready bit | **READY/STALL** | as row 3: each accessor reads only the unit's committed state; `unit_ready_fn_` callback deleted | compliant | 4 |
...
| 9 | `CoalescingUnit::evaluate` calls `ldst_.fifo_pop()`, `gather_file_.claim()`, `cache_.process_load()`/`process_store()` mid-evaluate; also reads `cache_.is_stalled()` at top of its evaluate as a same-cycle backpressure signal | LdStUnit FIFO, gather buffer file, cache | command-style mutations + COMBINATIONAL stall read | command-style mutations into LdStUnit/gather/cache are documented "internal-subsystem-mutating commands"; the gather-buffer write-port boundary arbitrates via REGISTERED state owned by `LoadGatherBufferFile` (row 11). The `cache.is_stalled()` read is a Phase 9 COMBINATIONAL same-tick edge: cache's stall signal is combinationally driven from registered tag/write-buffer/pending_fill state, and `cache.evaluate()` runs before `coalescing.evaluate()` in tick order. Models a same-cycle stall handshake — not pipelined, since synthesizing the path on the target FPGA is straightforward at the cache scale (small flop-backed tag array). | gather-buffer port arbitration cleaned up (Phase 7); cache↔coalescing stall edge formally classified COMBINATIONAL with call-site comment in `coalescing_unit.cpp` (Phase 9) | compliant (Phase 7 + 9) | 7, 9 |
| 10 | `L1Cache` external observable surface: REGISTERED scratch (`pending_fill_`, four `last_*_event_` slots) flipped by `commit()`, accessors return `current_*`. COMBINATIONAL same-tick (`stalled_`, `stall_reason_`) — single slot, reset at top of `evaluate`, observed mid-tick by coalescing (row 9) and post-commit by `record_cycle_trace`. Internal hardware state (`tags_`, `mshrs_`, `write_buffer_`) intentionally direct-mutated. | `TimingModel::record_cycle_trace` (post-commit, REGISTERED reads); `CoalescingUnit::evaluate` (same-tick, COMBINATIONAL stall read); cache itself; cache test expectations | mixed boundary discipline | Phase 9 carve-out: scratch trace events and the cross-cycle `pending_fill_` carrier go through `current_/next_` pairs so `record_cycle_trace` (which runs after `cache.commit()`) sees committed state via the `current_*` slot. Stall flags stay single-slot COMBINATIONAL because pipelining would cost a cycle of backpressure latency every stall transition (~5% on `embedding_gather`) without a hardware payoff at this cache scale. Internal MSHR/tag/write-buffer mutation stays direct because it has no cross-stage observers — only `L1Cache` itself touches it, and `test_cache.cpp` / `test_cache_mshr_merging.cpp` assert MSHR/tag state synchronously. | compliant (Phase 9); cache external boundary now classified per-field, internals carved out by design | 9 |
| 15 | `L1Cache` calls `mem_if_.submit_read()` / `submit_write()` (mid-evaluate writes into mem_if's `in_flight_`); `L1Cache::handle_responses` reads `mem_if_.has_response()` / `get_response()` (mid-evaluate dequeue from `responses_`) | `ExternalMemoryInterface` (FixedLatencyMemory and DRAMSim3Memory implementations) | external-memory request/response queue mutations | trusted-subsystem carve-out, mirroring row 10. Tick order is `cache.evaluate -> ... -> mem_if.evaluate`, so a request submitted in cycle N is processed by `mem_if.evaluate` at the end of the same cycle (one cycle of memory latency consumed at submit time). Likewise responses produced by `mem_if.evaluate` in cycle N are dequeued by `cache.handle_responses` at the top of cycle N+1, since `mem_if.evaluate` runs after `cache.evaluate`. The DRAMSim3 backend has tighter constraints (write-region FIFO bounds; see `dramsim3_memory.cpp:67`) that make REGISTERED queue mutation invasive without a clear payoff. | compliant by design (carve-out, Phase 9 documented) | 9 |
| 11 | `LoadGatherBufferFile::try_write` writes only `next_port_claimed_` (single shared flag, not per-buffer; models §5.3 "one line-to-gather-buffer extraction per cycle"). `L1Cache::handle_responses`, `drain_secondary_chain_head`, and `process_load` HIT path all funnel through `try_write`. | `LoadGatherBufferFile::try_write` reads the live `next_port_claimed_` (combinational first-writer-wins) | intra-cycle write-port arbitration | first-writer-wins via the shared `next_port_claimed_` flag; `commit()` clears the flag at end-of-cycle so the next tick starts unclaimed. FILL > secondary > HIT priority is encoded by tick ordering: `cache_->evaluate()` runs at the top of the non-panic tick (FILL via `handle_responses`, secondary via `drain_secondary_chain_head`); `coalescing_->evaluate()` runs later in the tick (HIT via `process_load`). | tick order: `gather_file.commit` clears the flag at end-of-cycle; first writer in tick N+1 sees `next_port_claimed_ == false` and wins | compliant (Phase 7) | 7 |
...
- **Pre-evaluate setters that latch live state from another stage.**
  ...
  reorders calls. Expose the signal as a `const` `ready_out()` accessor
  on the consumer that reads only its own committed state.
...
- **Phase 9** (landed): Cache external boundary discipline. The cache subsystem
  ...
  - The cache↔`mem_if` boundary (`submit_read`/`submit_write`/`has_response`/
...
- **Consolidation pass** (landed): hygiene cleanup after Phase 8.
  - `compute_ready()` and the `ready_out_` cache slot deleted
    everywhere. READY/STALL signals are now exposed as `const`
  ...
  - `is_ready()` removed from `ExecutionUnit` and all five concrete
    ...
    `ready_out()`. `OperandCollector::is_free()` removed; callers use
    `ready_out()`.
