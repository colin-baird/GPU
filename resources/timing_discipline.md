...
| 6 | `OperandCollector::accept()` (mutates `busy_`, `cycles_remaining_`, `current_instr_` synchronously) | `OperandCollector::evaluate` and `OperandCollector::is_free()` | opcoll busy/cycles/instr | should be **REGISTERED** next/current | `accept()` mutates committed state mid-cycle; `commit()` only flips `current_output_` | non-compliant | 2 |
| 7 | `ALUUnit`/`MultiplyUnit`/`DivideUnit`/`TLookupUnit`/`LdStUnit::accept()` (mutate `pending_input_`, `result_buffer_` directly); `WritebackArbiter::evaluate` calls `consume_result()` which mutates `result_buffer_.valid` synchronously on the source unit | downstream `evaluate()` (writeback arbiter sees `has_result()`); `is_ready()` consumed by scheduler | per-unit `pending_input_`, `result_buffer_` | should be **REGISTERED** (`next_pending_input_`, `next_result_buffer_`); `commit()` flips | every unit's `commit()` is empty or trivial; mutations bypass the cycle boundary | non-compliant | 1 |
