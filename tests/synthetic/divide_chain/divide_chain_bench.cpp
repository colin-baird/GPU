...
constexpr uint32_t kOutputBase = 0x00001000;
...
int main(int argc, char* argv[]) {
    try {
        ...
        config.start_pc = load_program(model, DIVCHAIN_KERNEL_ELF);
        // Pre-zero the output region so the post-run read is unambiguous.
        for (uint32_t lane = 0; lane < 32; ++lane) {
            model.memory().write32(kOutputBase + lane * 4, 0);
        }
        ...
        // Verify functional div-by-zero result: every lane wrote x15 = -1.
        bool divzero_ok = true;
        for (uint32_t lane = 0; lane < 32; ++lane) {
            const int32_t v = static_cast<int32_t>(model.memory().read32(kOutputBase + lane * 4));
            if (v != -1) { divzero_ok = false; break; }
        }

        // Per spec §4.6: each DIV occupies the divide unit for exactly
        // `kDivLatency` cycles, busy-until-complete (no pipelining). With a
        // dependent chain across iterations + 1 div-by-zero per warp, total
        // DIV instructions = num_warps × (iterations + 1).
        const uint64_t expected_div_instructions = static_cast<uint64_t>(opts.num_warps) * (opts.iterations + 1);
        // div_stats.busy_cycles should equal kDivLatency × instructions
        // exactly: only one DIV resident at a time (single-slot iterative
        // unit), so back-to-back issues do not overlap.
        const uint64_t expected_div_busy = expected_div_instructions * kDivLatency;

        const bool ok_div_count = stats.div_stats.instructions == expected_div_instructions;
        const bool ok_div_busy  = stats.div_stats.busy_cycles == expected_div_busy;
        ...
        if (opts.json_output) {
            ...
            extra << ",\n  \"benchmark\": \"divide_chain\""
                  << ",\n  \"iterations\": " << opts.iterations
                  << ",\n  \"expected_div_instructions\": " << expected_div_instructions
                  << ",\n  \"expected_div_busy_cycles\": " << expected_div_busy
                  << ",\n  \"match_div_count\": " << (ok_div_count ? "true" : "false")
                  << ",\n  \"match_div_busy\": " << (ok_div_busy ? "true" : "false")
                  << ",\n  \"divzero_result_ok\": " << (divzero_ok ? "true" : "false");
            json.insert(pos, extra.str());
            std::cout << json;
        } else {
            std::cout << "divide_chain completed\n";
            std::cout << "  warps: " << opts.num_warps
                      << "  iterations: " << opts.iterations
                      << "  cycles: " << timing.cycle_count() << "\n";
            std::cout << "  div_instructions: " << stats.div_stats.instructions
                      << " (expected " << expected_div_instructions << ")"
                      << (ok_div_count ? " OK" : " MISMATCH") << "\n";
            std::cout << "  div_busy_cycles: " << stats.div_stats.busy_cycles
                      << " (expected " << expected_div_busy << ")"
                      << (ok_div_busy ? " OK" : " MISMATCH") << "\n";
            std::cout << "  div_avg_latency: "
                      << (stats.div_stats.instructions == 0 ? 0.0
                          : static_cast<double>(stats.div_stats.busy_cycles)
                            / static_cast<double>(stats.div_stats.instructions)) << " cyc/instr\n";
            std::cout << "  divzero_result: " << (divzero_ok ? "OK (-1 as per RV32M)" : "MISMATCH") << "\n";
        ...
        return (ok_div_count && ok_div_busy && divzero_ok) ? 0 : 4;
    } catch (const std::exception& e) {
        std::cerr << "divide_chain_bench error: " << e.what() << "\n";
        return 1;
    }
}
